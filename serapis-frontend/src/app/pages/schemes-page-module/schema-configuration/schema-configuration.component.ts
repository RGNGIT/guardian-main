...
import { Schema, SchemaCondition, SchemaField } from '@app/models/schema';
import {
  AbstractControl,
  FormBuilder,
  FormControl,
...
import { SchemaEntity } from '@app/enums/schema';
import { IPFSService } from '@app/services/ipfs.ervice';
import * as moment from 'moment';
import {FieldTypesDictionary} from "@app/models/field-types-dictionary";
...
export class SchemaConfigurationComponent implements OnInit {
  ...
  getId(type: 'default' | 'measure' | 'schemas'): string {
    switch (type) {
      ...
      case 'schemas':
        return String(
          this.types.length + this.measureTypes.length + this.schemaTypes.length
        );
    }
  }
  ...
  ngOnChanges(changes: SimpleChanges): void {
    ...
    if (this.dataForm) {
      ...
      if (this.system) {
        this.defaultFields = new FormControl(
    ...
    if (this.schemasMap && topicId) {
      this.schemas = this.schemasMap[topicId]?.filter(s => {
        if (this.value?.document?.$id) {
          const isInDefs = Object.keys(s.document?.$defs || {}).includes(
            this.value.document.$id
          );
  ...
  onIfConditionFieldChange(condition: ConditionControl, field: FieldControl) {
  ...
  private ifFormatValue(condition: ConditionControl, field: FieldControl) {
    ...
    const isNumber =
      ['number', 'integer'].includes(type.type) || type.format === 'duration';
